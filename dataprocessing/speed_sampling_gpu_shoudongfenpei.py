...
def point_rand_sample_bound_points(numsamples, dim, v, f, offset, margin, mesh):
    ...
    # 1. 勘探与分类
    # 为了充分勘探，我们使用比总样本数还大的点量进行撒点
    num_exploration_points = int(numsamples * 2) 
    ...
    # 2. 制定预算
    # 【核心配置】在这里，我们手动为我们关心的物体分配采样名额
    BUDGET = {
        2:    int(numsamples * 0.7),  # 小石头 (ID=2)，分配70%的预算
        1339: int(numsamples * 0.15), # 大石头A (ID=1339)，分配15%的预算
        1743: int(numsamples * 0.15), # 大石头B (ID=1743)，分配15%的预算
    }
    
    # 3. 按预算执行细采样
    X_list, Y_list = budgeted_fine_sampling(seed_points_by_category, BUDGET, triangles_obs, dim, offset, margin)

    if not X_list:
        print("警告：按预算采样阶段未能生成任何点。")
        return np.array([]), np.array([])

    # 4. 合并最终结果
    X_final = torch.cat(X_list, 0)
    Y_final = torch.cat(Y_list, 0)

    # 如果生成的点超过总样本数，进行随机下采样以满足数量要求
    if len(X_final) > numsamples:
        indices = torch.randperm(len(X_final))[:numsamples]
