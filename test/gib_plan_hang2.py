...
#can be changed according to your .pt file
womodel.load('./Experiments/Gib_multi_hang2/Model_Epoch_10000_ValLoss_1.221157e-01.pt')#
...
gib_id = 0
v, f = igl.read_triangle_mesh("datasets/gibson_hang2/"+str(gib_id)+"/mesh_z_up_scaled.off")        
print(gib_id)
...
B = np.load("datasets/gibson_hang2/"+str(gib_id)+"/B.npy")
...
for ii in range(5):
    ...
    #start_goal = np.array([[2,-7,-4,6,-2,2.5]])
    #start_goal = np.array([[8,-7,-4,-6,-2,10]])
...
mesh = o3d.io.read_triangle_mesh("datasets/gibson_hang2/"+str(gib_id)+"/mesh_z_up_scaled.off")
