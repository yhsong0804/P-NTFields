#8.8 Focal Loss + 预热学习率1063行左右（删去一行+定义参数+新逻辑）
import matplotlib
import numpy as np
import math
import random
...
class Model():
    ...
    def train(self):
        ...
        for epoch in range(1, self.Params['Training']['Number of Epochs']+1):
            ...
            # 定义预热和衰减的超参数
            warmup_epochs = 50
            normal_lr = self.Params['Training']['Learning Rate'] # 例如: 1e-3
            warmup_start_lr = 1e-5 # 从一个非常低的学习率开始预热

            if epoch <= warmup_epochs:
                # --- 预热阶段 ---
                # 线性地将学习率从 warmup_start_lr 增加到 normal_lr
                # 为了避免除以零的错误，我们做一个小小的保护
                if warmup_epochs > 1:
                    progress = (epoch - 1) / (warmup_epochs - 1)
                else:
                    progress = 1.0
                current_lr = warmup_start_lr + progress * (normal_lr - warmup_start_lr)
            else:
                # --- 正常训练与衰减阶段 ---
                # 这里是你之前的学习率衰减逻辑，现在它只在预热结束后生效
                decay_start_epoch = 8000
                if epoch > decay_start_epoch:
                    # 注意：这里的衰减逻辑应该基于 normal_lr，而不是写死的 1e-3
                    current_lr = np.clip(normal_lr * (1 - (epoch - decay_start_epoch) / 1000.), a_min=5e-4, a_max=normal_lr)
                else:
                    current_lr = normal_lr
