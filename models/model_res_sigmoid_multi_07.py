#idea：动态调整采样点分布 注意修改train文件的引用路径
        ...
        for epoch in range(1, self.Params['Training']['Number of Epochs'] + 1):
            ...
            if epoch % 500 == 0 and epoch > 0:
                if len(error_stats) > 0:
                    print("High-error sample grid detected, resampling for next round ...")
                    np.save(self.Params['ModelPath'] + f"/error_points_epoch_{epoch}.npy", np.array(error_stats))
                    error_stats = []

            while True:
                total_train_loss = 0
                total_diff = 0
                for i, data_tuble in enumerate(dataloader, 0):
                    data = data_tuble[0].to(self.Params['Device'])
                    B = data_tuble[1].to(self.Params['Device'])
                    ...
                    for ii in range(inner_size):
                        ...
                        # === 动态采样记录：仅在每N epoch记录高loss点 ===
                        if epoch % 500 == 0 and epoch > 0:
                            wv_abs = wv.abs().detach().cpu().numpy()
                            idx_top = wv_abs.argsort()[-10:]  # 取前10高loss
                            err_points = batch_points.detach().cpu().numpy().reshape(-1, batch_points.shape[-1])
                            for idx in idx_top:
                                error_stats.append(err_points[idx])
